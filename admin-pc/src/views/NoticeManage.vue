...
async function fetchList() {
  ...
  try {
    ...
    if (keyword.value.trim()) {
      ...
      list = list.filter((n: NoticeSummary) => n.title.toLowerCase().includes(kw))
...
const form = ref({ title: '', content: '', attachment: '' })
const fileInput = ref<HTMLInputElement | null>(null)
const uploading = ref(false)
const submitting = ref(false)

function handleCreate() {
  form.value = { title: '', content: '', attachment: '' }
...
async function handleFileSelect(e: Event) {
  ...
  try {
    ...
    fd.append('file', file)
    const res = await request.post('/files/upload', fd, { headers: { 'Content-Type': 'multipart/form-data' } })
    form.value.attachment = res.data.name
  } catch {}
...
function handleEdit(row: NoticeSummary) {
  editForm.value = { id: row.id, title: row.title, content: '' }
  editVisible.value = true
  // 加载完整内容
  getNoticeDetail(row.id).then(res => {
...
async function handleEditSave() {
  ...
  try {
    await updateNotice(editForm.value.id, { title: editForm.value.title, content: editForm.value.content })
    ElMessage.success('修改成功')
    editVisible.value = false
    fetchList()
  } catch {} finally { editSubmitting.value = false }
}
...
// ========== 删除通知 ==========
async function handleDelete(row: NoticeSummary) {
  try {
    await ElMessageBox.confirm('确定删除该通知？此操作不可恢复。', '确认删除', {
      type: 'warning',
  ...
  <div class="notice-page">
    ...
    <div class="toolbar">
      ...
      <div style="display:flex;gap:8px">
        <el-input v-model="keyword" placeholder="搜索标题..." size="small" style="width:200px" clearable @input="onSearch" />
    ...
    <el-dialog v-model="dialogVisible" title="新建通知" width="600px">
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item label="正文">
          <el-input v-model="form.content" type="textarea" :rows="6" placeholder="请输入通知正文" />
        </el-form-item>
        <el-form-item label="附件">
          <input type="file" ref="fileInput" @change="handleFileSelect" style="display:none" />
          <el-button size="small" @click="($refs.fileInput as any).click()">选择文件</el-button>
          <span v-if="uploading" style="margin-left:8px">上传中...</span>
          <span v-else-if="form.attachment" style="margin-left:8px;color:#07c160">{{ form.attachment }}</span>
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <!-- 编辑通知弹窗 -->
    <el-dialog v-model="editVisible" title="编辑通知" width="600px">
      <el-form :model="editForm" label-width="80px">
        <el-form-item label="标题">
          <el-input v-model="editForm.title" placeholder="请输入通知标题" />
        </el-form-item>
        <el-form-item label="正文">
          <el-input v-model="editForm.content" type="textarea" :rows="6" placeholder="请输入通知正文" />
        </el-form-item>
      </el-form>
      <template #footer>
